...
describe("useRemixClient", () => {
    ...
    test("inizializzazione: nessun contratto trovato", async () => {
        ...
        expect(result.current.consoleMessages.some(m => m.includes("No contracts found in contracts folder."))).toBe(true);
    });
    ...
    test("executeMutations: messaggio se nessun mutator selezionato", async () => {
        ...
        expect(result.current.consoleMessages.some(m => m.includes("Please select at least one mutation operator."))).toBe(true);
    });
    ...
    test("executeTesting: salva il report", async () => {
        ...
        await act(async () => {
            await result.current.executeTesting(
                { testingFramework: "hardhat", testingTimeOutInSec: 30 },
                testFiles
            );
        });
    ...
    test("eventi fileManager: aggiornano la lista contratti", async () => {
        ...
        client.fileManager.readdir
            .mockResolvedValueOnce(mockContractsFirst)
            .mockResolvedValueOnce(mockContractsSecond);
    ...
    test("executeTesting: testing file not found", async () => {
        ...
        await act(async () => {
            await result.current.executeTesting(
                { testingFramework: "truffle", testingTimeOutInSec: 30 },
                testFiles
            );
        });
